...
def verify_wrf_inputs(inp, bdy, do_assert=True):
    """Verify that the WRF boundary data agree with initial condition data
    along boundaries at Time=0
    """
    ...
    def my_assert(OK):
        if do_assert:
            assert OK, 'checkfields returned False'
    ...
    for bdyoff in range(bdywidth):
        ...
        my_assert(checkfields(inp['MU'].isel(west_east=bdyoff),
                              bdy['MU_BXS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(inp['MU'].isel(west_east=-(bdyoff+1)),
                              bdy['MU_BXE'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(inp['MU'].isel(south_north=bdyoff),
                              bdy['MU_BYS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(inp['MU'].isel(south_north=-(bdyoff+1)),
    ...
    for bdyoff in range(bdywidth):
        print('bdy_width=',bdyoff)
        my_assert(checkfields(
                get_mass_weighted('U',inp,west_east_stag=bdyoff),
                bdy['U_BXS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('U',inp,west_east_stag=-(bdyoff+1)),
                bdy['U_BXE'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('U',inp,south_north=bdyoff),
                bdy['U_BYS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('U',inp,south_north=-(bdyoff+1)),
                bdy['U_BYE'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('V',inp,west_east=bdyoff),
                bdy['V_BXS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('V',inp,west_east=-(bdyoff+1)),
                bdy['V_BXE'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('V',inp,south_north_stag=bdyoff),
                bdy['V_BYS'].isel(Time=0,bdy_width=bdyoff)))
        my_assert(checkfields(
                get_mass_weighted('V',inp,south_north_stag=-(bdyoff+1)),
    ...
    for bdyvarn in vars_to_check:
        ...
        for bdyoff in range(bdywidth):
            ...
            my_assert(checkfields(
                    get_mass_weighted(inpvarn,inp,west_east=bdyoff),
                    bdy[bdyvarn+'_BXS'].isel(Time=0,bdy_width=bdyoff)))
            my_assert(checkfields(
                    get_mass_weighted(inpvarn,inp,west_east=-(bdyoff+1)),
                    bdy[bdyvarn+'_BXE'].isel(Time=0,bdy_width=bdyoff)))
            my_assert(checkfields(
                    get_mass_weighted(inpvarn,inp,south_north=bdyoff),
                    bdy[bdyvarn+'_BYS'].isel(Time=0,bdy_width=bdyoff)))
            my_assert(checkfields(
                    get_mass_weighted(inpvarn,inp,south_north=-(bdyoff+1)),
